...
//	if (!empty($api_key)) {
		...
		$finalSearchPageUrl = esc_url(home_url('/?s=' . urlencode($search_value)));
			...
			<div class="search-suggestion-box">
				<div class="popular-suggestion results-list">
					...
					<div class="desktop-btn-box red-btn">
						<a href="' . $finalSearchPageUrl . '">' . __('Show All Products', 'web2application') . '</a>
					</div>
				</div>
...
function send_product_data_to_remote_server() {
	...
	?><script>console.log('engage action triggered ');</script><?php
	// Get the app_id
	$path = W2A_APP_DATA_DIR . 'web2appdata.json';

	if (file_exists($path)) {
		$json_data = file_get_contents($path);
?><script>console.log('engage web2appdata exist');</script><?php
		if ($json_data) {
			$data = json_decode($json_data, true);
			if ($data && isset($data['app_id'])) {
				$app_id = $data['app_id'];
?><script>console.log('engage appid : <?php echo addslashes($app_id); ?>');</script><?php
			}
		}
	}
		...
		if (is_product()) {
			?><script>console.log('engage is product ');</script><?php
	} else {
			?><script>console.log('engage no product ');</script><?php
	}
		
	if (isset($_COOKIE['aUserCookieID'])) {
			?><script>console.log('engage is aUserCookieID ');</script><?php
	} else {
			?><script>console.log('engage no aUserCookieID ');</script><?php
	}
	
	
//check why this DEV for the search
	if (isset($_GET['dev'])) {
...
//send the product hit
		if (is_product() && isset($_COOKIE['aUserCookieID'])) {
?><script>console.log('engage is product and have cookie');</script><?php
			if (is_singular()) {
?><script>console.log('engage singilar');</script><?php
				$product_data = api_call_data();
				$file_path = plugin_dir_path(__FILE__) . 'product.json';

				$existing_data = [];
				if (file_exists($file_path)) {
?><script>console.log('engage products.json exist');</script><?php
...
?><script>console.log('engage save product to product.json');</script><?php					
				}

				if (!empty($existing_data)) {
					if (count($existing_data) >= 50) {
