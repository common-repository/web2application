...
//echo "OrderID after 1 section : " . $last_order_number;
	...
	$safeOrderLoopStop = $current_order_id + 1000;
...
// Get the app_id and send abandone carts to server
	$path = W2A_APP_DATA_DIR . 'web2appdata.json';

	if (file_exists($path)) {
		$json_data2 = file_get_contents($path);

		if ($json_data2) {
			$data2 = json_decode($json_data2, true);
			if ($data2 && isset($data2['app_id'])) {
				$app_id = $data2['app_id'];
			}
		}
	}
...
//echo "MYAPPID = : ".$app_id;
send_abandoned_carts_to_server($app_id);
...
//send abandone carts to server
function send_abandoned_carts_to_server($app_id) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'w2a_abandoned_carts';

    // Get abandoned carts that haven't been sent and were created more than 24 hours ago
    ...
    if ($carts) {
        foreach ($carts as $cart) {
            $data = array(
                ...
                'user_cookie_id' => $cart->user_cookie_id,
            ...
            $url = 'https://web2application.com/w2a/engage/engage-api/exept-wp-abandone-carts.php?appnumber=' . $app_id;
