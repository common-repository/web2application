...
function create_abandoned_cart_table() {
    ...
    $sql = "CREATE TABLE IF NOT EXISTS $table_name (
        ...
        UNIQUE KEY user_cart (user_id, user_cookie_id)
    ) $charset_collate;";
...
function web2app_addMenu() {
	...
	if(is_admin()){
		...
		add_menu_page('Web2apllication', $label_webapplication, 'publish_posts', 'Web2apllication-main', 'w2a_general_settings');

		add_submenu_page('Web2apllication-main','Send Push', $label_send_push ,'publish_posts', 'send-push', 'w2a_sendPushPage');

		add_submenu_page('Web2apllication-main','App Members Club', $label_members_club ,'publish_posts', 'web2application-app-members-club', 'w2a_members_club');

		add_submenu_page('Web2apllication-main','Tab Menu Links', $label_tab_menu_links ,'publish_posts','web2application-tab-menus','w2a_tab_menus');

		add_submenu_page('Web2apllication-main','Marketing Tools', $label_marketing_tools ,'publish_posts','web2application-marketing-tools','w2a_marketing_tools');

		add_submenu_page('Web2apllication-main','Premium Settings', $label_premium_settings ,'publish_posts','web2application-premium-settings','w2a_premium_settings');

		add_submenu_page('Web2apllication-main','Web Push Settings', $label_web_push_settings ,'publish_posts','web2application-web-push-settings','w2a_web_push_settings');

		add_submenu_page('Web2apllication-main','Web2application site', $label_webapplication_site ,'publish_posts','Web2apllication-main-2','web2applicationMainTab');
...
function w2a_is_ip_allowed($allowed_ips) {
    // Get the client IP address
    if (!empty($_SERVER['HTTP_CLIENT_IP'])) {
        $w2a_ip_address = $_SERVER['HTTP_CLIENT_IP'];
    } elseif (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
        $w2a_ip_address = $_SERVER['HTTP_X_FORWARDED_FOR'];
    } else {
        $w2a_ip_address = $_SERVER['REMOTE_ADDR'];
    }

    // Split the X-Forwarded-For string if there are multiple IPs
    $w2a_ip_address = explode(',', $w2a_ip_address)[0];
    
    // Trim any whitespace
    $w2a_ip_address = trim($w2a_ip_address);
    
    // Debugging: print the detected IP address
    error_log("Detected IP: $w2a_ip_address");

    // Check if the IP address is in the allowed list
    if (in_array($w2a_ip_address, $allowed_ips)) {
        return array('status' => true, 'ip' => $w2a_ip_address);
    }
    return array('status' => false, 'ip' => $w2a_ip_address);
}

// Register a custom REST API endpoint - nir . works well
//get orders

	add_action('rest_api_init', function () use ($w2a_allowed_ips) {
		register_rest_route('web2application/v1', '/getorders', array(
			'methods' => 'POST',
			'callback' => 'we2application_engage_get_categories',
			'permission_callback' => function () use ($w2a_allowed_ips) {
				// Check if the IP address is allowed
				$check = w2a_is_ip_allowed($w2a_allowed_ips);
				if ($check['status']) {
					return true;
				} else {
					return new WP_Error('rest_forbidden', esc_html__('IP ' . $check['ip'] . ' does not have permissions to access this endpoint.'), array('status' => 403));
				}
			}
		));
	});
...
//get products
	add_action('rest_api_init', function () use ($w2a_allowed_ips) {
		register_rest_route('web2application/v1', '/getproducts', array(
			'methods' => 'GET',
			'callback' => 'we2application_engage_get_products',
			'permission_callback' => function () use ($w2a_allowed_ips) {
				// Check if the IP address is allowed
				$check = w2a_is_ip_allowed($w2a_allowed_ips);
				if ($check['status']) {
	...
	function we2application_engage_get_products($request) {
		// Your custom data retrieval logic goes here

		include 'api/get_products.php';

		// Return the response - its makes a but and printed a product...
		//return rest_ensure_response($data);
	}

// update app details after update
// Function to fetch data and save it

//w2a_update_app_data_and_save(trim($w2a_options['w2a_api_key']));

function w2a_update_app_data_and_save($w2a_api_key) {
	if( trim($w2a_api_key) != '' ) {
		$url = 'https://www.web2application.com/w2a/api-process/get_app_data.php?api_domain='.$_SERVER['SERVER_NAME'].'&api_key='.trim($w2a_api_key).'&version=new';
...
function w2a_handle_plugin_update($upgrader_object, $options) {
    global $w2a_options;

    // Check if the current action is 'update' and the type is 'plugin'
    if ($options['action'] == 'update' && $options['type'] == 'plugin') {
        // Loop through the plugins being updated
        if (isset($options['plugins'])) {
            foreach ($options['plugins'] as $plugin) {
                // Correct path to your plugin's main file
                if ($plugin == 'web2application/web2application.php') {
                    // Check and update the app data
                    if (isset($w2a_options['w2a_api_key']) && trim($w2a_options['w2a_api_key']) != '') {
                        error_log('Updating app data for plugin: ' . $plugin);
                        w2a_update_app_data_and_save(trim($w2a_options['w2a_api_key']));
                    }
                }
            }
...
add_action('upgrader_process_complete', 'w2a_handle_plugin_update', 10, 2);
